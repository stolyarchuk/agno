...
escalation_manager_agent = Agent(
    ...
    instructions=[
        ...
        "When a user reports an issue, always send it to the #testing Slack channel with all relevant details using the send_message toolkit function.",
...
feedback_collector_agent = Agent(
    ...
    instructions=[
        ...
        "When a user provides feedback or suggests a feature, use the Slack tool to send it to the #testing channel using the send_message toolkit function.",
...
customer_support_team.print_response(
    "I want to create an educational agent that has tons of data on teacher's video ,pdfs and text files, how can i do that using agno", stream=True
)
